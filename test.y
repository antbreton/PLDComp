...
declaration : declaration_droite separateur_decl PV {$$->affectation=NULL; $$ = $2; $$->variables->push_back($1); for(int i=0; i<$$->variables->size();i++) {(*$$->variables)[i]->setType($1->getType());}} // On crée un vecteur de variables qui auront toutes le type de la première déclaration
            | declaration_droite separateur_decl EGAL_AFFECTATION expression PV 
...
$$->affectation->setValeur($4);

 if($$->variables->size()==1) {
$$->affectation->setIdentifiant(new Identifiant ( new string ($1->getIdentifiant() ) ) );
} else {
$$->affectation->setIdentifiant(new Identifiant ( new string ( (*$$->variables)[$$->variables->size()-2]->getIdentifiant() ) ));
}  
} ; /* on recupere la liste dans $2 et on rajoute le premier $1 en dernier. Ensuite on recupere le type de $1 et on l'affecte a tout ceux de la liste. Ensuite on affecte la valeur au dernier de la liste(donc l'avant dernier du vecteur) ou bien a la seul variable*/
...
contenu_bloc :contenu_bloc declaration {if($2!=NULL){ 
if($2->variables != NULL)
$$->ajouterListeVariable($2->variables); 

cout<<$2->affectation<<endl; 
if($2->affectation != NULL) $$->AjouterInstr($2->affectation);}} // On ajoute la liste des variables résultantes de la déclaration dans le bloc
							// Dans ce cas, nous devons traiter le cas ou l'instruction est elle même un bloc et lui désigné le bloc courant comme ancêtre
